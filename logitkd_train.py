...
from utils.my_utils import smart_optimizer
from utils.loss_opti import smart_logit_kd_loss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    compute_loss_ota = ComputeLossOTA(student)  # init loss class
    compute_loss_kd = smart_logit_kd_loss(teacher, student, kd_loss)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 11) % (
            'Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'kdbox', 'kdcls', 'kdobj', 'total', 'labels', 'img_size'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss_kd(tea_pred,
                                                   stu_pred,
                                                   targets.to(device),
                                                   imgs)  # loss scaled by batch_size

                total_items = torch.cat((loss_items, loss)).detach()
        ...
        if rank in [-1, 0]:
            ...
            tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss',  # train loss
                    'train/kdbox_loss', 'train/kdcls_loss', 'kdobj_loss',  # kd loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kd-loss', type=str, default=None)
